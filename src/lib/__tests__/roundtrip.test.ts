...
import { checkFidelity, DecodeError, decodeDocument, encodeDocument } from "../decode";
...
describe.skipIf(!hasRealFile)(`round-trip fidelity (${REAL_FILE})`, () => {
  ...
  it("rates a non-canonically encoded but complete file as equivalent, not lossy", () => {
    ...
    const normalised = encodeDocument(decodeDocument(readRealFile()));
  ...
  it("survives a decode/encode/decode cycle unchanged", () => {
    const bytes = readRealFile();
    const once = encodeDocument(decodeDocument(bytes));
...
describe("decode guards", () => {
  it("rejects data that is not protobuf at all", () => {
    ...
    expect(() => decodeDocument(new TextEncoder().encode("<xml>not a pro7 file</xml>")))
      .toThrow(DecodeError);
    try {
      decodeDocument(new TextEncoder().encode("<xml>not a pro7 file</xml>"));
    } catch (e) {
